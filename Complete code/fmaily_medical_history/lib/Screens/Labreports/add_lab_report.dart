...
class AddLabReport extends StatefulWidget {
  const AddLabReport({Key? key}) : super(key: key);

  @override
  _AddLabReportState createState() => _AddLabReportState();
}

class _AddLabReportState extends State<AddLabReport> {
                      ...
                      if(imageFile==null)
                      {
                        ...
                        try{
                          ...
                          if (result.isNotEmpty && result[0].rawAddress.isNotEmpty){
                            uploadLabReport(imageFile!, reportTitle.text, lName.text, context);
                          }
                          ...
                          await saveLabReportDataLocally(imageFile!.path, reportTitle.text, lName.text);
                          Navigator.pop(context);
                          showDialog(context: context, builder: (context){
...
Future<void> saveLabReportDataLocally(String imagePath,String reportTitle,String lName) async {
  final SharedPreferences prefs = await SharedPreferences.getInstance();
  final List<String> localData = prefs.getStringList('localLabReportData') ?? [];

  localData.add('$reportTitle,$lName,$imagePath');
  await prefs.setStringList('localLabReportData', localData);
}
Future<void> uploadLocalLabReportData() async {
  final SharedPreferences prefs = await SharedPreferences.getInstance();
  final List<String> localData = prefs.getStringList('localLabReportData') ?? [];
  ...
  for (final data in localData) {
    ...
    final String imagePath = values[2];
    final File imageFile = File(imagePath);
    uploadLocallyLabReport(imageFile, reportTitle, lName);
  }

  // Clear local data after successful upload
  await prefs.remove('localLabReportData');
}
