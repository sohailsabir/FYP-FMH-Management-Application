...
class _ProfileState extends State<Profile> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: loading?spinkit:SingleChildScrollView(
        child: Column(
          children: [
            Stack(
              children: [
                Container(
                  ...
                  child: Column(
                    children:  [
                      ...
                      !check?Container():Text(snapshot['Email'],
                        style: const TextStyle(
                          color: Colors.white,
                          fontSize: 18,
                        ),)
                          ...
                          if(imageFile!=null){
                            try{
                              ...
                              if (result.isNotEmpty && result[0].rawAddress.isNotEmpty){
                               editUserProfileWithImage(fName.text, lName.text,date.text, bloodType!, duration.years.toString(), context, imageFile!, snapshot['Image']).then((value){
                                 setState(() {
                                   reload();
                                   imageFile=null;
                                 });
                            ...
                            editUserProfile(fName.text, lName.text, date.text, bloodType!, duration.years.toString(), context).then((value){
                              setState(() {
                                reload();
                                imageFile=null;
                              });
  ...
  reload()async{
    ...
    await FirebaseFirestore.instance.collection('User').doc(uid).get().then((data){
      setState(() {
        snapshot=data;
