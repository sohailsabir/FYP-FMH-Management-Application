...
class FamilyMedicationCustomPDF extends StatefulWidget {
  const FamilyMedicationCustomPDF({Key? key,required this.familyId}) : super(key: key);
  final String familyId;

  @override
  _FamilyMedicationCustomPDFState createState() => _FamilyMedicationCustomPDFState();
}

class _FamilyMedicationCustomPDFState extends State<FamilyMedicationCustomPDF> {
  List pdfdata = [];
  var userData;
  ...
  getMedicationData(var sDate,var eDate)async{
    ...
    await FirebaseFirestore.instance.collection('User').doc(uid).collection('Family member').doc(widget.familyId).collection('Medication').get().then((value){
      print(value.size);
