...
import 'package:fmaily_medical_history/Screens/FamilyMembers/FamilyMemberMedication/add_family_member_medication.dart';
import 'package:fmaily_medical_history/Screens/FamilyMembers/FamilyMemberMedication/family_medication_pdf.dart';
import 'package:fmaily_medical_history/Screens/FamilyMembers/FamilyMemberMedication/family_member_all_medication.dart';
import 'package:fmaily_medical_history/Screens/FamilyMembers/FamilyMemberMedication/family_member_current_medication.dart';
import 'package:font_awesome_flutter/font_awesome_flutter.dart';
import 'package:intl/intl.dart';

import '../../../Components/showMessage/customDialog.dart';
import '../../../Controller/firebase_authentication.dart';
import '../../../Services/PDF/medication_pdf.dart';
class FamilyMedicationsScreen extends StatefulWidget {
  const FamilyMedicationsScreen({Key? key,required this.familyMemberId,required this.medicationId}) : super(key: key);
  final String familyMemberId;
  final String medicationId;

  @override
  _FamilyMedicationsScreenState createState() => _FamilyMedicationsScreenState();
}
...
class _FamilyMedicationsScreenState extends State<FamilyMedicationsScreen> {
  ...
  late List<Widget> screen=[
    FamilyCurrentMedicationScreen(medicineId: widget.medicationId,familyMemberId: widget.familyMemberId),
    FamilyAllMedicationScreen(medicineId: widget.medicationId,familyMemberId: widget.familyMemberId),
  ];
  getMedicationData(int num)async{
    pdfdata=[];
    final uid=await getUserId();
    await FirebaseFirestore.instance.collection('User').doc(uid).collection('Family member').doc(widget.familyMemberId).collection('Medication').get().then((value){
      if(num==1)
      {
                ...
                actions: [
                  Center(
                    child: ElevatedButton(onPressed: (){
                      Navigator.pop(context);
                      Navigator.push(context, MaterialPageRoute(builder: (context)=>FamilyMedicationCustomPDF(familyId: widget.familyMemberId)));
                    },
                      style: ElevatedButton.styleFrom(
                        backgroundColor: kprimarycolor,
      ...
      floatingActionButton: SizedBox(
        ...
        child: Visibility(
          ...
          child: FloatingActionButton(
            ...
            onPressed: (){
              Navigator.push(context, MaterialPageRoute(builder: (context)=> AddFamilyMedicineScreen(medicationId: widget.medicationId,familyMemberId: widget.familyMemberId,)));
            },
            child: const Icon(
              Icons.add,
              size: 40,
